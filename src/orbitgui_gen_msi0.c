/* orbitgui_gen_msi0.c - gui for msi frame generation from orbits */
 ...
 */
int /* void /**/
orbitgui_gen_msi0_ok_CB( ORBIT *orbit, FLDMENU *fldMenu)
{
IMGFRM *lclimgfrm;

  /* DPR1( "Entering orbitgui_gen_msi0_ok_CB // "); /**/
  orbit_utc2et( orbit->_agen._utc, &orbit->_agen._EtStart);
  orbit->_agen._typeAgen = AGEN_TYPE_TIMING;

  free( fldMenu);

  /* DPR1( "before gen_msi0_frames // "); /**/
  lclimgfrm = gen_msi0_frames( orbit);  /* actually generate the frames */
  /* DPR1( "before add_gen_frames // "); /**/
  add_gen_frames( orbit, lclimgfrm);
  /* DPR1( "leaving orbitgui_gen_msi0_ok_CB // "); /**/
...
int /* void /**/
orbitgui_gen_msi0_cancel_CB( ORBIT *orbit, FLDMENU *fldMenu)
{
...
/**********************************************************************/
/* orbitgui_create_gen_msi0_dialog -- create dialog for frame generation
 */
void
orbitgui_create_gen_msi0_dialog( Widget buttonw, ORBIT *orbit)
{
int           i, numflds;
char          **labels;
char          lclstr[20];
...
char          *txt0;
static icount;
static char wildcard[] = { "*.agenmsitmg" };
...
#define LENALLOC (1+( (UTCLEN>IDLEN)?UTCLEN:IDLEN ))
  ...
  /* DPR(( stderr, "in create_gen_msi0_dialog malloc'ed fldMenu=%08x\n"
         , (int)fldMenu)); /**/
  ...
  sprintf( fm->fld_txt0, "MSI autogen # %d", icount++);
  ...
  fm->type = FLD_DBL; fm->fld_dbl = &orbit->_agen._TimePerMSIFrm; fm++;
  /* fm->type = FLD_INT; fm->fld_int = &orbit->_agen._TimePerMSIFrm; fm++; */
  ...
  fm->type = FLD_END;
  fm->client_call = orbitgui_gen_msi0_cancel_CB;
